...
export function DocumentEditorModal({
  ...
  const documentLabel = labelFor(detail?.kind);
  ...
  async function handleSave() {
    if (!contentHtml.trim()) {
      setError(`${documentLabel} content cannot be empty.`);
      return;
    }
    ...
    try {
      const result = await documentsService.update(doc.id, {
        title: title.trim(),
        contentHtml,
        changeSummary: `Edited ${documentLabel.toLowerCase()}`,
      });
      ...
      setError(err instanceof Error ? err.message : `Failed to save ${documentLabel.toLowerCase()}.`);
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-[#1F2937] bg-[#111827] shadow-2xl">
        <div className="flex items-start justify-between border-b border-[#1F2937] px-5 py-4">
          <div className="min-w-0 flex-1">
            <h2 className="truncate pr-4 text-[17px] font-semibold text-white">Edit {documentLabel}</h2>
            <p className="mt-1 text-[12px] text-[#9CA3AF]">
              Save your edits as the latest {documentLabel.toLowerCase()} version. Downloads will use the updated version automatically.
            </p>
          </div>
          <button
            type="button"
            ...
            <div className="space-y-4">
              ...
              {saved && (
                <div className="rounded-lg border border-[#14532D] bg-[#14532D]/10 px-4 py-3 text-[13px] text-[#86EFAC]">
                  {documentLabel} saved. The latest version is ready to preview and download.
                </div>
              )}
              ...
              <div>
                ...
                <RichTextEditor
                  ...
                  placeholder={`Edit your ${documentLabel.toLowerCase()} here…`}
                  minRows={16}
        ...
        <div className="flex flex-col gap-3 border-t border-[#1F2937] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-[12px] text-[#6B7280]">
            Rich text formatting is preserved when you save and when you download the {documentLabel.toLowerCase()} PDF.
          </div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setError(err instanceof Error ? err.message : `Failed to download ${documentLabel.toLowerCase()}.`);
                } finally {
