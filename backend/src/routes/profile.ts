...
router.get("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const skills = await query<{ skill_name: string; years_experience: number }>(
      `SELECT skill_name, years_experience FROM user_skills WHERE user_id = $1 ORDER BY skill_name`,
      [req.userId]
    );
...
const updateProfileSchema = z.object({
  ...
  currentTitle:    z.string().max(200).trim().optional(),
...
router.put("/", validate(updateProfileSchema), async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    // Update account_users fields
    if (data.firstName || data.lastName || data.location !== undefined ||
        data.currentTitle !== undefined || data.linkedinUrl !== undefined) {
      await query(
        `UPDATE account_users SET
         ...
         WHERE id = $1`,
        [req.userId, data.firstName ?? null, data.lastName ?? null,
         data.location ?? null, data.currentTitle ?? null, data.linkedinUrl ?? null]
      );
    }
    ...
    await query(
      `INSERT INTO user_profiles (user_id, professional_summary, years_experience, remote_only, min_salary_usd, max_salary_usd)
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         updated_at           = NOW()`,
      [req.userId, data.summary ?? null, data.yearsExperience ?? null,
       data.remoteOnly ?? null, data.minSalary ?? null, data.maxSalary ?? null]
    );
