...
router.get("/preferences", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json(prefs ? {
      ...
      mirrorJobKeywords: prefs.mirror_job_keywords,
      prioritizeRecentExperience: prefs.prioritize_recent_experience,
      keepBulletsConcise: prefs.keep_bullets_concise,
      avoidFirstPerson: prefs.avoid_first_person,
      emphasizeLeadership: prefs.emphasize_leadership,
...
const prefsSchema = z.object({
  ...
  mirrorJobKeywords:        z.boolean().optional(),
  prioritizeRecentExperience: z.boolean().optional(),
  keepBulletsConcise:       z.boolean().optional(),
  avoidFirstPerson:         z.boolean().optional(),
  emphasizeLeadership:      z.boolean().optional(),
  ...
  try {
    await query(
      `INSERT INTO user_preferences (
         ...
         only_rephrase,
         mirror_job_keywords,
         prioritize_recent_experience,
         keep_bullets_concise,
         avoid_first_person,
       ...
       VALUES (
         ...
         $26,
         COALESCE($27, 'Playfair Display'),
         COALESCE($28, 'Source Sans 3'),
         COALESCE($29, '#2563EB'),
         COALESCE($30, 'modern'),
         COALESCE($31, 'balanced'),
         COALESCE($32, false)
       )
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         mirror_job_keywords        = COALESCE($20, user_preferences.mirror_job_keywords),
         prioritize_recent_experience = COALESCE($21, user_preferences.prioritize_recent_experience),
         keep_bullets_concise       = COALESCE($22, user_preferences.keep_bullets_concise),
         avoid_first_person         = COALESCE($23, user_preferences.avoid_first_person),
         emphasize_leadership       = COALESCE($24, user_preferences.emphasize_leadership),
         ai_custom_roles            = COALESCE($25::text[], user_preferences.ai_custom_roles),
         ai_default_instructions    = COALESCE($26, user_preferences.ai_default_instructions),
         resume_title_font          = COALESCE($27, user_preferences.resume_title_font),
         resume_body_font           = COALESCE($28, user_preferences.resume_body_font),
         resume_accent_color        = COALESCE($29, user_preferences.resume_accent_color),
         resume_template            = COALESCE($30, user_preferences.resume_template),
         resume_density             = COALESCE($31, user_preferences.resume_density),
         use_legacy_resume_preferences_for_ai = COALESCE($32, user_preferences.use_legacy_resume_preferences_for_ai),
         updated_at                 = NOW()`,
      [
        req.userId,
        ...
        d.onlyRephrase ?? null,
        d.mirrorJobKeywords ?? null,
        d.prioritizeRecentExperience ?? null,
        d.keepBulletsConcise ?? null,
        d.avoidFirstPerson ?? null,
        d.emphasizeLeadership ?? null,
